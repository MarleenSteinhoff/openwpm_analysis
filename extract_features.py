...
def get_cookies(db_file, id_urls_map=tuple(), max_rank=None):
    ...
    if CRAWL_NAME in ["2016-03", "2016-04", "2016-08", "2016-09", "2017-01", "2017-02",
        ...
        query = f"""SELECT js.visit_id, js.name, js.path, js.isHttpOnly, js.creationTime, js.expiry, js.value, 
                                js.host, sv.site_url, sv.visit_id FROM profile_cookies as js LEFT JOIN site_visits as sv
                                        ON sv.visit_id = js.visit_id WHERE js.visit_id IN {format(id_urls_map)} 
                                        """
...
def all_tables_to_csv(crawl_db_path):
    ...
    for table_name in tables:
        ...
        if table_name in ["crawl", "task", "sqlite_sequence", "xpath", "local_storage"]:
            continue
        table = pd.read_sql("SELECT * from %s LIMIT 1;" % table_name, db, parse_dates={"time_stamp": {"errors": "ignore"}, "creationTime": {"errors": "ignore"}})
        filename = join(OUT_DIR, "%s_%s" % (CRAWL_NAME, table_name + ".csv"))
        table.to_csv(filename, index_label="index", sep="\t")
    cursor.close()
    db.close()
...
if __name__ == '__main__':
    ...
    if SELECTED_IDS_ONLY:
        ...
        extract_features_chunks_linear(crawl_db_path, out_csv, selected_visit_ids, MAX_RANK)
